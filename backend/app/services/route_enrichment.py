...
def _expand_detail(detail: list, num_points: int) -> list[Optional[object]]:
    """Convertit un path_detail GraphHopper ([from_idx, to_idx, value], ...)
    en une valeur par point du tracé, pour un usage direct côté frontend."""
...
def _leg_boundaries(coordinates: list, snapped_waypoints: list) -> list[int]:
    """Index, dans `coordinates`, de chaque waypoint demandé (dans l'ordre).

    GraphHopper renvoie `snapped_waypoints` : les coordonnées de chaque waypoint
    demandé une fois "accrochées" au réseau routier, qui apparaissent telles
    quelles dans `points.coordinates` (vérifié empiriquement contre l'instance
    GraphHopper réelle). Le curseur avance de façon monotone pour rester correct
    même sur un trajet qui repasse près d'un point déjà visité (boucle/aller-retour).
    """
    boundaries: list[int] = []
    cursor = 0
