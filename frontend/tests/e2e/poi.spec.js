...
import { confirmDelete, mapPointAt, openTab } from "./helpers.js";
...
test("point d'intérêt : clic droit -> formulaire -> marqueur + liste -> suppression", async ({ page, request }) => {
  try {
    ...
    const point = await mapPointAt(page, 48.86, 2.33);
    ...
    await popup.locator("input[type=text]").press("Enter");
    ...
    // Deux clics : confirmation intégrée au bouton, sans boîte de dialogue navigateur.
    let dialogFired = false;
    page.on("dialog", (dialog) => {
      dialogFired = true;
      dialog.dismiss();
    });
    await confirmDelete(poiItem.locator('button[data-action="delete"]'));
    expect(dialogFired).toBe(false);
    await expect(page.locator("#poi-list li")).toHaveCount(0);
    await expect(page.locator(".leaflet-marker-icon")).toHaveCount(0);
  } finally {
    // Nettoyage même en cas d'échec : un point restant fausserait les autres tests.
