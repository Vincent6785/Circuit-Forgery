...
// Coordonnées exactes en mer près du Cotentin ayant réellement déclenché le bug
// en usage réel (PointNotFoundException GraphHopper, cf. README). Attention :
// un point "en mer" pris au hasard peut en réalité se faire snapper sur une
// ligne de ferry indexée par GraphHopper (constaté avec un premier point dans le
// golfe de Gascogne, qui a renvoyé un itinéraire de 1000+ km au lieu d'échouer) —
// ce point précis a été vérifié via curl comme déclenchant bien un 422.
...
test("point sans route à proximité : message inline, pas de popup navigateur", async ({ page }) => {
  ...
  // L'app doit rester utilisable : après avoir effacé les points, un nouveau calcul
  // valide doit fonctionner et faire disparaître le message d'erreur.
  await page.locator("#clear-route-btn").click();
  // Zoom sur Paris pour espacer ces deux points en pixels (cf. route-planning.spec.js).
